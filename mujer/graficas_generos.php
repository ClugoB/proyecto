...
$stmt = $conn->prepare("SELECT SUM(cantidad_hombres) as total_hombres, SUM(cantidad_mujeres) as total_mujeres FROM form_mujeres");
$stmt->execute();
$result = $stmt->fetch(PDO::FETCH_ASSOC);

$total_hombres = $result['total_hombres'] ?? 0;
$total_mujeres = $result['total_mujeres'] ?? 0;
...
<body>
    ...
    <div class="content">
        ...
        <main>
            <div class="header">
                <div class="left">
                    <h1>Total de Géneros</h1>
                </div>
            </div>
            <!-- GRÁFICA DE TOTAL DE HOMBRES Y MUJERES-->
            <div>
        ...
        <option value="polarArea">Área Polar</option>
            ...
            <table class="table">
                ...
                <tbody>
                    <tr>
                        <td>Hombres</td>
                        <td><?php echo htmlspecialchars($total_hombres); ?></td>
                    </tr>
                    <tr>
                        <td>Mujeres</td>
                        <td><?php echo htmlspecialchars($total_mujeres); ?></td>
                    </tr>
                </tbody>
            </table>
        </main>
...
let myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            data: [<?php echo json_encode($total_hombres); ?>, <?php echo json_encode($total_mujeres); ?>],
            backgroundColor: [
                'rgba(75, 192, 192, 0.8)',
                'rgba(255, 99, 132, 0.8)'
            ],
