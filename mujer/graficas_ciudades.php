...
$stmt = $conn->prepare("SELECT ciudad_ubicado, SUM(cantidad_hombres) as total_hombres, SUM(cantidad_mujeres) as total_mujeres FROM form_mujeres GROUP BY ciudad_ubicado");
$stmt->execute();
            ...
            <table class="table">
                ...
                <tbody>
                    <?php for($i = 0; $i < count($ciudades); $i++): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($ciudades[$i]); ?></td>
