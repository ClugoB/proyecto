...
$stmt = $conn->prepare("SELECT parroquia_ubicado, SUM(cantidad_hombres) as total_hombres, SUM(cantidad_mujeres) as total_mujeres FROM form_mujeres GROUP BY parroquia_ubicado");
$stmt->execute();
...
foreach ($datos as $dato) {
    $parroquias[] = $dato['parroquia_ubicado'];
...
<body>
    ...
    <div class="content">
        ...
        <main>
            ...
            <!-- GRÁFICA DE MUJERES Y HOMBRES EN PARROQUIAS-->
            <div>
    ...
    <select id="chartType">
        ...
        <option value="polarArea">Área Polar</option>
            ...
            <table class="table">
                ...
                <tbody>
                    <?php for($i = 0; $i < count($parroquias); $i++): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($parroquias[$i]); ?></td>
    ...
    function renderChart() {
        if (myChart) {
            myChart.destroy(); // Destruir el gráfico existente si lo hay
        }

        myChart = new Chart(ctx, {
            type: chartType,
            data: {
                labels: <?php echo json_encode($parroquias); ?>,
                datasets: [{
