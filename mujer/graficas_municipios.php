...
$stmt = $conn->prepare("SELECT municipio_ubicado, SUM(cantidad_hombres) as total_hombres, SUM(cantidad_mujeres) as total_mujeres FROM form_mujeres GROUP BY municipio_ubicado");
$stmt->execute();
...
foreach ($datos as $dato) {
    $municipios[] = $dato['municipio_ubicado'];
...
<body>
    ...
    <div class="content">
        ...
        <main>
            ...
            <!-- GRÁFICA DE MUJERES Y HOMBRES EN MUNICIPIOS-->
            <div>
            ...
            <table class="table">
                ...
                <tbody>
                    <?php for($i = 0; $i < count($municipios); $i++): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($municipios[$i]); ?></td>
